...
- @space_between_groups@ _(boolean)_ := whether to require space between groups of selectors. Defaults to @true@
- @min_group_size@ _(int)_ := specifies the maximum number of properties that may be in a group before space is required around it (default: 3. Ignored if @space_between_groups@ is @false@)
- @order@ _(hash)_ := a hash of the configured groups. The groupname should be vaguely human-readable to provide sane lint messages =)


h3. Specifying groups

Groups are specified as a YAML hash, in the order in which they should appear in your SASS. Each individual member is an array of properties that may appear in that group.
For example:

<pre><code lang="yaml">order:
  tables:
    - table-layout
    - border-collapse
